...
def main():
    ...
    for kk_time in range(args.seedstart, args.seedstart+1):
        ...
        lemniscate = LinearAverage(args.low_dim, ndata, args.nce_t, args.nce_m).to(device)


        if args.synthesis:
            print ("running synthesis")
            criterion = BatchCriterionFour(1, 0.1, args.batch_size, args).to(device)
        elif args.multiaug:
            print ("running cvpr")
            criterion = BatchCriterion(1, 0.1, args.batch_size, args).to(device)
        else:

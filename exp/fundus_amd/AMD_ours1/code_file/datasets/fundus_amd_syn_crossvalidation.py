...
class traindataset(data.Dataset):
    ...
    def __init__(self, root, transform=None, train=True, args=None):
        ...
        images_path = np.genfromtxt(self.root_dir + 'random_list.txt', dtype='str')
        ...
        if self.train:
            ...
            for i in range(0, len(train_path)):
                image = cv2.imread(self.root_dir + "fundus/" + train_path[i].split("/")[-1])
                ...
                if self.synthesis:
                    image = cv2.imread(self.root_dir + "FFA_syn_AMD/" + train_path[i].split("/")[-1][:-4] + ".png")
            ...
            for i in range(0, len(test_path)):
                image = cv2.imread(self.root_dir + "fundus/" + test_path[i].split("/")[-1])
                self.train_dataset.append(image)
